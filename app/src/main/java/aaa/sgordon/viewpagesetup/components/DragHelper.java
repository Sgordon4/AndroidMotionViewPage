...
public class DragHelper {
	...
	public float getMediaHeight(ImageView media) {
	...
	private void setChildInteractivity(ViewGroup viewGroup, boolean enabled) {
		for (int i = 0; i < viewGroup.getChildCount(); i++) {
			...
			if (child instanceof ViewGroup)
				setChildInteractivity((ViewGroup) child, enabled);
		}
	}
			...
			case MotionEvent.ACTION_MOVE:
				...
				if(!isActive()) {
					...
					if (motionLayout.getProgress() == 0f) {
						//Are we dragging down? If so, don't interfere with other gestures
						if(deltaY >= 0)
							return false;
					}
				}
